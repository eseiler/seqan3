...
std::vector<std::string> const current_compression_extensions = [] ()
    {
        std::vector<std::string> tmp;
        for (auto const & ext : seqan3::detail::compression_extensions)
            tmp.emplace_back(ext);
        return tmp;
    }();
...
TEST(validator_test, input_file)
{
    ...
    { // single file
        ...
        {  // allow compression extension
            seqan3::input_file_validator my_validator{std::vector{std::string{"fasta"}},
                                                      std::vector{std::string{"gz"}, std::string{"bgzf"}}};
        ...
        {  // allow compression extension automatically by a file
            seqan3::input_file_validator<dummy_file> my_validator{};
            EXPECT_NO_THROW(my_validator(gz_tmp_name.get_path()));
            EXPECT_NO_THROW(my_validator(bgzf_tmp_name.get_path()));
            EXPECT_THROW(my_validator(gz_no_ext_tmp_name.get_path()), seqan3::validation_error);
    ...
    { // get help page message
        ...
        parser.add_positional_option(path, "desc",
                                     seqan3::input_file_validator{formats, current_compression_extensions});
...
TEST(validator_test, output_file)
{
    ...
    { // single file
        ...
        {  // allow compression extension automatically with dummy_file
            seqan3::input_file_validator<dummy_file> my_validator{};
            EXPECT_NO_THROW(my_validator(gz_tmp_name.get_path()));
            EXPECT_NO_THROW(my_validator(bgzf_tmp_name.get_path()));
            EXPECT_THROW(my_validator(gz_no_ext_tmp_name.get_path()), seqan3::validation_error);
    ...
    // get help page message
    {
        ...
        test_accessor::set_terminal_width(parser, 80);
        parser.add_positional_option(path, "desc",
                                     seqan3::output_file_validator{formats, current_compression_extensions});
