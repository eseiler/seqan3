...
auto position =
    seqan3::detail::persist | std::views::transform([](auto && res) { return res.reference_begin_position(); });
...
TYPED_TEST(search_test, on_result_invocation)
{
    ...
    seqan3::configuration const cfg = seqan3::search_cfg::on_result{
        [&](auto && search_result) -> void { actual_positions.push_back(search_result.reference_begin_position()); }};
