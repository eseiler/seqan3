...
template <std::ranges::viewable_range resource_t,
    ...
    requires std::ranges::forward_range<resource_t> && std::invocable<algorithm_t,
...
private:
    ...
    void find_next_non_empty_bucket()
    {
        ...
        buffer_it = std::find_if(buffer_it, buffer_end_it, [](auto const & buffer) { return !buffer.empty(); });
